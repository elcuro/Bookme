...
class BookmesController extends BookmeAppController {
       ...
       public function add() {
              ...
              if (!empty($this->request->data)) {
                     if ($res = $this->Bookme->place($this->request->data)) {
                            $this->Session->setFlash(__('Your booking was successfuly sended'));
                     }
              }
              ...
              if (!in_array($type, $this->bookingNodeTypes)) {
                     $this->Session->setFlash(__('Wrong booking node type'));
              ...
              $params = array(
                  'conditions' => array(
                      ...
                      /* 'cache' => array(
                        'name' => 'bookme_nodes_type_' . $type,
                        'config' => 'nodes_index'
                        ) */
              );
              $nodes = $this->Node->find('all', $params);
              if (empty($nodes)) {
                     $this->Session->setFlash(__('No valid units for booking!'));
                     return $this->redirect(array(
                                 'controller' => 'nodes',
                                 'action' => 'promoted'
                             ));
              ...
              $this->set('title_for_layout', __('Booking'));
       ...
       public function admin_view($id = false) {
              ...
              if (!$id || !($bookme = $this->Bookme->findById($id))) {
                     $this->Session->setFlash(__('Invalid booking ID'), 'default', array('class' => 'error'));
                     return $this->redirect(array(
                                 'controller' => 'bookmes',
                                 'action' => 'admin_all'
                             ));
       ...
       public function admin_edit($id = false) {
              ...
              if (!empty($this->request->data)) {
                     if ($res = $this->Bookme->save($this->request->data)) {
                            $this->Session->setFlash(__('Your booking was successfuly updated'), 'default', array('class' => 'success'));
                            return $this->redirect(array(
                                'controller' => 'bookmes',
                                'action' => 'admin_all'
                            ));
                     } else {
                            $this->Session->setFlash(__('Error during booking saving!'), 'default', array('class' => 'error'));                         
                     }
              }
